...
interface AnalysisResult {
  ...
  url: string;
  timestamp: Date;
  screenshots: Record<string, ScreenshotVariant>;
  elements: {
...
export class PuppeteerAIAnalyzer {
  ...
  async captureAndAnalyzePage(url: string, testId: string): Promise<AnalysisResult> {
    if (!this.browser) {
  ...
  private async setupPageMonitoring(page: Page, analysis: AnalysisResult): Promise<void> {
  ...
  private async captureMultipleScreenshots(page: Page, testId: string): Promise<Record<string, ScreenshotVariant>> {
    const screenshots: Record<string, ScreenshotVariant> = {};
  ...
  private async captureScreenshot(page: Page, testId: string, variant: string, options: any): Promise<ScreenshotVariant> {
  ...
  private async extractDetailedElements(page: Page): Promise<AnalysisResult['elements']> {
      ...
      elements.navigation = Array.from(document.querySelectorAll('nav a, [role="navigation"] a'))
        .map(link => {
          ...
          return {
            ...
            href: linkEl.href,
            active: linkEl.classList.contains('active') || linkEl.getAttribute('aria-current') === 'page'
          };
        });
  ...
  private async performAccessibilityAnalysis(page: Page): Promise<{ issues: AccessibilityIssue[]; score: number }> {
    return await page.evaluate(() => {
      const accessibility = {
  ...
  private async testResponsiveness(page: Page, testId: string): Promise<{ breakpoints: Record<string, any>; issues: any[] }> {
    const breakpoints = [
    ...
    for (const breakpoint of breakpoints) {
      ...
      await new Promise(resolve => setTimeout(resolve, 1000)); // Allow for responsive adjustments
  ...
  private async capturePerformanceMetrics(page: Page): Promise<Record<string, any>> {
    return await page.evaluate(() => {
  ...
  private async extractAIAnalysisData(page: Page): Promise<any> {
    return await page.evaluate(() => {
